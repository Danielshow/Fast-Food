...
var FoodListController = function () {
  ...
  _createClass(FoodListController, [{
    ...
    value: function postFood(req, res, next) {
      ...
      _index2.default.query('INSERT INTO foodlist(food, price, image) VALUES($1,$2,$3)', [req.body.food.toLowerCase(), req.body.price.toLowerCase(), imagePath], function (err) {
        if (err) {
          return next(err);
        }
        return res.status(200).json({
    ...
    value: function updateFood(req, res, next) {
      ...
      var imagePath = _shared2.default.imagePicker(req);
      _index2.default.query('UPDATE foodlist SET food=$1,price=$2,image=$3 WHERE id=$4', [req.body.food.trim().toLowerCase(), req.body.price, imagePath, id], function (err) {
        if (err) {
          return next(err);
        }
        return res.status(200).json({
