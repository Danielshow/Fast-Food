...
class AuthController {
  register(req, res, next) {
    ...
    const params = [req.body.name.toLowerCase(), req.body.email.trim().toLowerCase(), password, req.body.address.trim().toLowerCase(), 'user'];
  ...
  adminRegister(req, res, next) {
    const password = bcrypt.hashSync(req.body.password, 10);
    const params = [req.body.name.toLowerCase(), req.body.email.toLowerCase(), password, req.body.address.toLowerCase(), 'admin'];
    db.query('INSERT INTO users(name, email, password, address, roles) VALUES($1,$2,$3,$4,$5)', params, (err) => {
      if (err) {
        return next(err);
      }
